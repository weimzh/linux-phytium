...
static const unsigned int x100_primary_formats[] = {
...
static uint64_t x100_primary_formats_modifiers[] = {
...
static const unsigned int x100_cursor_formats[] = {
	DRM_FORMAT_ARGB8888,
};

void x100_dc_hw_vram_init(struct phytium_display_private *priv, resource_size_t vram_addr,
				   resource_size_t vram_size)
{
	...
	config = phytium_readl_reg(priv, group_offset,
				   X100_GPU_ADDRESS_TRANSFORM_SRC_ADDR);
	if (config)
		phytium_writel_reg(priv, config, group_offset,
				   X100_GPU_ADDRESS_TRANSFORM_SRC_ADDR);

	config = phytium_readl_reg(priv, group_offset,
				   X100_GPU_ADDRESS_TRANSFORM_SIZE);
	if (config)
		phytium_writel_reg(priv, config, group_offset,
				   X100_GPU_ADDRESS_TRANSFORM_SIZE);

	config = phytium_readl_reg(priv, group_offset,
				   X100_GPU_ADDRESS_TRANSFORM_DST_ADDR);
	if (config)
		phytium_writel_reg(priv, config, group_offset,
				   X100_GPU_ADDRESS_TRANSFORM_DST_ADDR);

	phytium_writel_reg(priv, (vram_addr & SRC_ADDR_MASK) >> SRC_ADDR_OFFSET,
			   group_offset, X100_DC_ADDRESS_TRANSFORM_SRC_ADDR);
	phytium_writel_reg(priv, (vram_size >> SIZE_OFFSET) | ADDRESS_TRANSFORM_ENABLE,
			   group_offset, X100_DC_ADDRESS_TRANSFORM_SIZE);
	config = phytium_readl_reg(priv, group_offset, X100_DC_ADDRESS_TRANSFORM_DST_ADDR);
	phytium_writel_reg(priv, config, group_offset, X100_DC_ADDRESS_TRANSFORM_DST_ADDR);
}

void x100_dc_hw_clear_msi_irq(struct phytium_display_private *priv, uint32_t phys_pipe)
{
	phytium_writel_reg(priv, MSI_CLEAR, priv->dcreq_reg_base[phys_pipe], X100_DCREQ_MSI_CLEAR);
}

void x100_dc_hw_config_pix_clock(struct drm_crtc *crtc, int clock)
{
...
void x100_dc_hw_disable(struct drm_crtc *crtc)
{
	struct drm_device *dev = crtc->dev;
	struct phytium_display_private *priv = dev->dev_private;
	struct phytium_crtc *phytium_crtc = to_phytium_crtc(crtc);
	...
	// reset dc
	config = phytium_readl_reg(priv, priv->dc_reg_base[phys_pipe], X100_DC_CLOCK_CONTROL);
	phytium_writel_reg(priv, config | SOFT_RESET, priv->dc_reg_base[phys_pipe],
			   X100_DC_CLOCK_CONTROL);
	phytium_writel_reg(priv, 0, priv->dc_reg_base[phys_pipe], X100_DC_CLOCK_CONTROL);
	do {
		config = phytium_readl_reg(priv, priv->dc_reg_base[phys_pipe], X100_DC_CLOCK_IDLE);
		if (config | IS_IDLE)
			break;
		mdelay(1);
		reset_timeout--;
	...
	/* reset pix clock */
	x100_dc_hw_config_pix_clock(crtc, 0);
	...
	/* reset dcreq */
	phytium_writel_reg(priv, DCREQ_PLAN_A, priv->dcreq_reg_base[phys_pipe], X100_DCREQ_PLAN);
	phytium_writel_reg(priv, 0, priv->dcreq_reg_base[phys_pipe], X100_DCREQ_CONTROL);
	phytium_writel_reg(priv, DCREQ_RESET, priv->dcreq_reg_base[phys_pipe], X100_DCREQ_RESET);
	msleep(20);
	phytium_writel_reg(priv, (~DCREQ_RESET)&DCREQ_RESET_MASK,
			   priv->dcreq_reg_base[phys_pipe], X100_DCREQ_RESET);
}

int x100_dc_hw_fb_format_check(const struct drm_mode_fb_cmd2 *mode_cmd, int count)
{
	int ret = 0;
...
void x100_dc_hw_plane_get_primary_format(const uint64_t **format_modifiers,
							 const uint32_t **formats,
							 uint32_t *format_count)
{
	*format_modifiers = x100_primary_formats_modifiers;
	*formats = x100_primary_formats;
	*format_count = ARRAY_SIZE(x100_primary_formats);
}

void x100_dc_hw_plane_get_cursor_format(const uint64_t **format_modifiers,
							const uint32_t **formats,
							uint32_t *format_count)
{
	*format_modifiers = NULL;
	*formats = x100_cursor_formats;
	*format_count = ARRAY_SIZE(x100_cursor_formats);
}
...
void x100_dc_hw_update_dcreq(struct drm_plane *plane)
{
	...
	if (phytium_plane->tiling[0] == FRAMEBUFFER_LINEAR) {
		phytium_writel_reg(priv, DCREQ_MODE_LINEAR,
				   group_offset, X100_DCREQ_PLANE0_CONFIG);
	} else {
		...
		phytium_writel_reg(priv, phytium_plane->iova[0] & 0xffffffff,
				   group_offset, X100_DCREQ_PLANE0_ADDR_START);
		phytium_writel_reg(priv, (phytium_plane->iova[0] + phytium_plane->size[0]) &
				   0xffffffff, group_offset, X100_DCREQ_PLANE0_ADDR_END);
		phytium_writel_reg(priv, config, group_offset, X100_DCREQ_PLANE0_CONFIG);
	}
}
...
void x100_dc_hw_update_primary_hi_addr(struct drm_plane *plane)
{
	...
	phytium_writel_reg(priv, (phytium_plane->iova[0] >> PREFIX_SHIFT) & PREFIX_MASK,
			   priv->dcreq_reg_base[phys_pipe], X100_DCREQ_PIX_DMA_PREFIX);
}
