				...
				struct mmc_request *mrq, u32 cnts, u32 offset)
{
	...
	if (!(host->caps & MMC_CAP_NONREMOVABLE) && readl(host->base + MCI_CARD_DETECT))
		return -ESHUTDOWN;
				 ...
				 struct mmc_request *mrq, struct mmc_command *cmd)
{
	...
	if (!(events & (MCI_RAW_INTS_CMD | MCI_INT_MASK_HTO))) {
		if (!(host->caps & MMC_CAP_NONREMOVABLE) && (events & MCI_RAW_INTS_RTO)
...
static irqreturn_t phytium_mci_irq(int irq, void *dev_id)
{
	...
	if (((events & event_mask) & MCI_RAW_INTS_CD) && !(host->caps & MMC_CAP_NONREMOVABLE)) {
...
int phytium_mci_common_probe(struct phytium_mci_host *host)
{
	...
	writel(MCI_SET_FIFOTH(0x2, 7, 8), host->base + MCI_FIFOTH);
	sdr_clr_bits(host->base + MCI_CLKENA, MCI_CLKENA_CCLK_ENABLE);
